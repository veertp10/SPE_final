...
FILE = "data.pth"
data = torch.load(FILE, map_location=device)  # Add map_location=device
...
# Create the model instance and load the state dictionary
model = NeuralNet(input_size, hidden_size, output_size).to(device)
model.load_state_dict(model_state)
model.eval()
...
def get_response():
    user_question = request.get_json()["msg"]
...
def predict():
    ...
    # print(response)
    logger.info(f"Predicted diseases: {response}")
